...
void finish(struct PCB* process) {
    ...
    finalclk = getClk();
    process->waitingTime = finalclk - process->arrivalT - process->runningT;

    printf("At time %d process %d finished arrival %d total %d remain %d wait %d\n",
           getClk(), process->id, process->arrivalT, process->runningT,
           process->runningT - process->tProcessed, process->waitingTime);

    totalbrust += process->runningT;
    TotalWaitingTime += process->waitingTime;

    // free its memory and log
    deallocateMemoryLog(process);

    // compute WTA + persist stats
    double WTA = (getClk() - process->arrivalT) * 1.0 / process->runningT;
    if (process->runningT == 0) WTA = 0;
    ...
    fprintf(SchedulerLog,
        ...
        getClk(), process->id, process->arrivalT,
        process->runningT - process->tProcessed,
        (finalclk - process->arrivalT), process->waitingTime,
        finalclk - process->arrivalT, WTA);

    // After freeing memory, try to admit any waiting processes (RR only)
    if (algorithm == 3) {
        while (!isEmpty(&waitingPQ)) {
            // pop the highest-priority-by-arrival waiting process
            ...
            if (allocated) {
                ...
                // still can't fit; put it back and stop trying
...
int main(int argc, char* argv[])
{
	...
	if (algorithm == 3)
		quantum = atoi(argv[2]);
	runningP = NULL;
	system("gcc Process/process.c -o process");
	switch (algorithm)
	{
	case 1:
		SJF();
